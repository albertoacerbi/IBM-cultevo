...
```{r 14-1, message = FALSE, warning = FALSE, echo = FALSE, fig.cap = "An example network."}
library(igraph)
...
```{r 14-6, echo=FALSE, message=FALSE, fig.cap="Different graph types (from left: circle, lattice, random, scale free, fully connected) vary in the density and distribution of edges between nodes."}
library(igraph)
parbefore <- par()$mar
par(mar=c(0,0,0,0))
par(mfrow=c(1,5))
...
```{r 14-10, fig.cap="A simple network with 10 vertices, which are connected by edges (arrows)."}
plot(net)
```
...
```{r 14-11, fig.cap="Example for two different network layouts, the grid and the ring."}
...
```{r 14-12, fig.cap="Using vertex coordinates preserves the exact layout of a network."}
par(mfrow = c(1,3))
...
```{r 14-13, fig.cap="Using syling arguments with the `plot()` function allows to manipulate the appearence of vertices, edges, and labels."}
plot(net, 
...
```{r 14-14, fig.cap="In this plot we indicate a node's age by colouring the node with a different colour (pale yellow: younger, red: older)"}
...
```{r 14-17, fig.cap="A cleaner version of the same network."}
...
```{r 14-19, fig.cap="To indicate different strength in the relationship of two nodes, we can vary the width of edge between them."}
plot(net, 
     vertex.color = V(net)$col, 
     vertex.label = NA,
     vertex.size = 9,
...
```{r 14-33, fig.cap="Example graph where nodes are coloured based on their betweenness centrality. "}
...
```{r 14-37, fig.cap="Gossip spreads first quickly and then more slowly throughout a population."}
ggplot(data_bnd) + 
...
```{r 14-39, fig.cap="Repeated runs of `gossip_model()` show that on average gossip spreads to less than $0.2$ of the population if individuals have on average 1 friend. For 2 and more friends gossip spreads to more than three quarter of the population."}
data_bnd <- bind_rows(data)
ggplot(data_bnd) +
...
```{r 14-40, fig.cap="Here we plot the end points of each simulation from the previous plot as a boxplot. This, again, shows that there is a dramatic difference of diffusion in networks with 1 to 3 friends, but far less in those with 5 and more friends."}
...
```{r 14-41, fig.cap="Speed at which gossip spreads depending on the average number of friends."}
...
```{r 14-45, fig.cap="In simple contagion the probability to acquire gossip scales linearly with the proportion of neighbours with gossip, whereas it increases superlinearly (here exponentially) in the case of complex contagion."}
...
```{r 14-50, fig.cap="There are no differences in the spread of gossip in clustered and random networks if it spreads based on simple contagion (left, $e=1$). However, if gossip spreads based on complex contagion (right, $e=2$), it spreads faster in clustered than in random networks."}
set.seed(1)
res <- bind_rows(
